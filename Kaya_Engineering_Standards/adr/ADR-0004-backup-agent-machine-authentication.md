...
The genuine external implementation is `antybubbs/Kaya-Docker-Agent`. Server and agent changes are maintained in separate cross-linked branches and must be reviewed and released as one coordinated protocol migration.
...
The coordinated implementation uses `cryptography` in both repositories, passes the normative vectors and direct interoperability, and has a Verified-with-conditions adversarial review. It resolves `KAYA-BAK-001` at the implementation checkpoint but does not authorise either draft PR to merge, approve a release, or change the separate OIDC/RDP decisions.
