...
- The provider authorization request uses a new state, nonce and PKCE verifier plus `prompt=login` and `max_age=0`.
...
- Administrators can revoke unused invitations. Creation, opening, rejection, redemption, revocation, link success and link failure remain auditable without raw tokens, passwords, TOTP values or full claims.
...
- Consuming before the external redirect prevents concurrent reuse but means a provider outage or abandoned flow requires a new invitation.
...
Upgrade revokes every unused legacy invitation because those rows have no trustworthy recipient/provider binding. Existing `ExternalIdentity` links and local accounts are preserved. Rollback restores the old schema but cannot safely restore revoked legacy invitation secrets; issue new invitations only after returning to a secure release.
