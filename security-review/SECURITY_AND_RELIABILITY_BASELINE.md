...
## Authorisation and administrative operations
...
| Retired shared evaluation mode | Cross-cutting mode, middleware and route-string policy removed for v0.27 | Only normal production authentication, permission, CSRF, validation and audit paths remain | Removal regression tests and existing production security suites cover the affected paths. |
| Audit events | `write_audit` across security and operational routes | Actor/action/entity and safe metadata persisted; audit failures are not uniformly fail-closed | Broad feature tests assert selected events. Central redaction and completeness need systematic validation. |
