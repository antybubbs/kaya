...
Migration runs before user traffic and background services. If backup, integrity, compatibility, or migration validation fails, Kaya aborts startup and reports the recovery location in its logs. Do not repeatedly edit or restart a failed database; preserve the logs and follow [Administrator Database Upgrades](../admin-database-upgrades.md).
