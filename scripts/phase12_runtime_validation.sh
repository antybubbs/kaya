...
"${compose[@]}" exec -T kaya python -c 'from app.core.security import hash_password; from app.db.session import SessionLocal; from app.models.models import RemoteManagerSetting, User, UserModulePermission; db=SessionLocal(); user=db.query(User).filter_by(email="synthetic@example.invalid").first(); user=user or User(email="synthetic@example.invalid", password_hash=hash_password("synthetic-admin-password"), first_name="Synthetic", last_name="Administrator", role="admin", is_active=True, authentication_type="local", role_source="local"); db.add(user); db.flush(); permission=db.query(UserModulePermission).filter_by(user_id=user.id, module_key="high_availability").first(); permission=permission or UserModulePermission(user_id=user.id, module_key="high_availability", allowed=True, created_by=user.id); db.add(permission); setting=db.query(RemoteManagerSetting).filter_by(key="high_availability_enabled").first(); setting=setting or RemoteManagerSetting(key="high_availability_enabled"); setting.value="1"; db.add(setting); db.commit(); db.close()'
