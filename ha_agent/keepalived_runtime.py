...
def validate_desired_configuration(action: dict) -> bytes:
    if (
        action.get("action_type") != "KEEPALIVED_APPLY"
        or action.get("dhcp_transition") != "DISABLED"
    ):
    ...
    if (
        ...
        or not content.startswith("# Managed by Kaya High Availability. Do not edit.\n")
        or len(content.encode()) > 32 * 1024
    ):
        raise KeepalivedRuntimeError(
            "Keepalived configuration is not a valid Kaya-managed document."
        )
    if any(marker in content for marker in FORBIDDEN_CONFIG_MARKERS):
        raise KeepalivedRuntimeError(
            "Keepalived configuration contains a forbidden directive or shell marker."
        )
    required = (
        "global_defs {",
        "script_user kaya-ha kaya-ha",
        "enable_script_security",
        "vrrp_script KAYA_DNS_",
        "vrrp_instance KAYA_HA_",
        "state BACKUP",
        "virtual_router_id ",
        "virtual_ipaddress {",
        'notify_master "/usr/lib/kaya-ha-agent/kaya_ha_transition.py master ',
        'notify_backup "/usr/lib/kaya-ha-agent/kaya_ha_transition.py backup ',
        'notify_fault "/usr/lib/kaya-ha-agent/kaya_ha_transition.py fault ',
    )
    if any(content.count(marker) != 1 for marker in required):
        raise KeepalivedRuntimeError(
            "Keepalived configuration does not match the fixed Kaya structure."
        )
    actual = hashlib.sha256(content.encode()).hexdigest()
    if actual != checksum:
        raise KeepalivedRuntimeError(
            "Keepalived configuration checksum verification failed."
        )
    return content.encode()


def apply_desired_keepalived(
    state, action: dict, *, runner: Callable = subprocess.run
) -> dict:
    ...
    if generation == current and state.get("keepalived_checksum") == action.get(
        "checksum"
    ):
        return {
            ...
            "backup_reference": state.get("keepalived_backup_reference"),
            "message": "Keepalived generation was already applied.",
        }
    pending = state.root / "pending-keepalived.conf"
    atomic_bytes(pending, validate_desired_configuration(action))
    try:
        completed = runner(
            ["sudo", "-n", HELPER_PATH, "apply", str(pending)],
            capture_output=True,
            text=True,
            timeout=30,
            check=False,
        ...
        raise KeepalivedRuntimeError(
            "The restricted Keepalived helper could not be executed."
        ) from exc
    try:
    ...
    if completed.returncode != 0 or not output.get("ok"):
        return {
            ...
            "message": str(
                output.get("message")
                or "Keepalived validation or activation failed and was rolled back."
            )[:1000],
        }
    ...
    return {
        ...
        "checksum": action["checksum"],
        "backup_reference": output.get("backup_reference"),
        "message": "Generated Keepalived configuration validated and activated.",
    }
...
def refresh_vip_state(state, *, runner: Callable = subprocess.run) -> None:
    ...
    try:
        completed = runner(
            ["sudo", "-n", HELPER_PATH, "status", str(vip).split("/", 1)[0]],
            capture_output=True,
            text=True,
            timeout=10,
            check=False,
