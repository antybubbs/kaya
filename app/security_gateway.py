...
app.add_middleware(
    ...
    session_cookie="secure_send_state",
    same_site="strict",
...
RECIPIENT_PATH_RE = re.compile(
    r"^/[A-Za-z0-9_-]{64}(?:/unlock|/download-package|/logout|/files/[1-9][0-9]*)?$"
)
STATIC_FILES = {
    "/assets/gateway.css": (
        Path("app/static/css/secure-send-gateway.css"),
        "text/css; charset=utf-8",
    ),
    "/assets/logo.png": (
        Path("app/static/brand/kaya-favicon-192-transparent.png"),
        "image/png",
    ),
...
def forbidden(*, https: bool = False) -> Response:
    return security_headers(
        PlainTextResponse("Forbidden", status_code=403), https=https
    )
...
def rejected(request: Request, reason: str, *, https: bool = False) -> Response:
    ...
    logger.warning(
        "Secure Send request rejected method=%s reason=%s",
...
def configured_gateway_hostname() -> str:
    ...
    try:
        hostname = (
            urlparse(
                get_site_setting(db, "secure_send_gateway_hostname") or ""
            ).hostname
            or ""
        ).lower()
...
def public_rate_limited(request: Request) -> bool:
    ...
    if len(PUBLIC_REQUESTS) > 10000:
        for stale_key in list(PUBLIC_REQUESTS)[:1000]:
            if (
                not PUBLIC_REQUESTS[stale_key]
                or now - PUBLIC_REQUESTS[stale_key][-1] >= 60
            ):
...
async def gateway_headers(request: Request, call_next):
    ...
    if request.url.path != "/healthz" and public_rate_limited(request):
        logger.warning(
            "Secure Send request rejected method=%s reason=public_rate_limit",
            request.method.upper(),
        )
        return security_headers(
            PlainTextResponse("Too Many Requests", status_code=429), https=https
        )
    if request.method == "POST":
        ...
        try:
            ...
            return rejected(request, "content_length_invalid", https=https)
...
def public_context(request: Request, **values):
    return {
        "brand": BRAND_CONTEXT,
        "csrf_token": request.session.get("recipient_csrf", ""),
        **values,
    }
...
def safe_state(row: SecureSendPackage | None) -> str:
    ...
    if row.revoked_at or row.status == "revoked":
        return "revoked"
    if row.deleted_at or row.status == "deleted":
        return "missing"
    if row.one_download_only and row.download_count >= 1:
        return "expired"
    if row.expires_at <= datetime.utcnow() or row.expired_at or row.status == "expired":
        return "expired"
    return "active"
...
def package_or_error(db: Session, token: str) -> SecureSendPackage:
    ...
    row = package_for_token(db, token)
    state = safe_state(row)
    if state != "active":
        logger.warning(
            "Secure Send request rejected reason=package_state state=%s", state
        )
        raise HTTPException(403, "Forbidden")
    return row
...
def gateway_css():
    return FileResponse(STATIC_FILES["/assets/gateway.css"][0], media_type="text/css")


@app.get("/assets/logo.png", include_in_schema=False)
@app.head("/assets/logo.png", include_in_schema=False)
def gateway_logo():
    return FileResponse(STATIC_FILES["/assets/logo.png"][0], media_type="image/png")
...
def attempt_limited(request: Request, row: SecureSendPackage) -> bool:
    now = time.monotonic()
    key = f"{client_ip(request)}:{row.id}"
    ATTEMPTS[key] = [value for value in ATTEMPTS.get(key, []) if now - value < 900]
    return len(ATTEMPTS[key]) >= 10 or bool(
        row.locked_until and row.locked_until > datetime.utcnow()
    )
...
def notify_sender(db: Session, row: SecureSendPackage, event: str) -> None:
    ...
    try:
        send_mail(
            db,
            row.sender.email,
            f"Secure Send package {event}",
            f"One of your Secure Send packages was {event}. Open Kaya to review its activity.",
        )
...
def recipient(access_token: str, request: Request, db: Session = Depends(get_db)):
    ...
    if not row.opened_at:
        ...
        row.status = "opened"
        record_activity(db, row, "opened", commit=False)
        db.commit()
        write_audit(
            db,
            row.sender,
        ...
        if row.notify_when_opened:
            notify_sender(db, row, "opened")
    session = active_recipient_session(db, row, request.cookies.get(SESSION_COOKIE))
    if not session:
        return templates.TemplateResponse(
            request,
            "secure_send_gateway_unlock.html",
            public_context(request, error=None, access_token=access_token),
        )
    key = package_key_from_application(row)
    _summary = decode_summary(row, key)
    files = decoded_files(db, row, key)
    note = decode_note(row, key)
    return templates.TemplateResponse(
        request,
        "secure_send_gateway_package.html",
        public_context(
            request,
            package=row,
            files=files,
            ...
            allow_vault_save=row.allow_vault_save and row.internal_recipient_id,
...
def unlock(
    ...
    try:
        authenticate_package(row, access_token, pin, passphrase)
    except SecureSendError:
        key = f"{client_ip(request)}:{row.id}"
        ATTEMPTS.setdefault(key, []).append(time.monotonic())
        row.failed_attempts += 1
        if row.failed_attempts >= 10:
            row.locked_until = datetime.utcnow() + timedelta(minutes=30)
        elif row.failed_attempts >= 5:
            row.locked_until = datetime.utcnow() + timedelta(minutes=5)
        db.commit()
        record_activity(db, row, "authentication_failed")
        write_audit(
            db,
            row.sender,
            "secure_send_authentication_failed",
            "secure_send_package",
            str(row.id),
            client_ip(request),
            category="security",
        ...
        return templates.TemplateResponse(
            ...
            public_context(
                request,
                error="The information entered is incorrect. Please check your PIN and passphrase and try again.",
                access_token=access_token,
            ),
            status_code=400,
        )
    ...
    db.commit()
    token, csrf, _ = start_recipient_session(db, row)
    request.session["recipient_csrf"] = csrf
    record_activity(db, row, "authenticated")
    write_audit(
        db,
        row.sender,
        "secure_send_authenticated",
        "secure_send_package",
        str(row.id),
        client_ip(request),
        category="security",
...
def authorised_download(
    db: Session, request: Request, access_token: str, csrf_token: str
) -> tuple[SecureSendPackage, bytes]:
    row = package_or_error(db, access_token)
    session = active_recipient_session(db, row, request.cookies.get(SESSION_COOKIE))
    if not session or not verify_session_csrf(session, csrf_token):
        raise HTTPException(
            403, "Secure Send is temporarily unavailable. Please try again later."
        )
    if row.one_download_only and row.download_count >= 1:
        raise HTTPException(
            410, "This secure package is no longer available for download."
        )
...
def mark_download(db: Session, request: Request, row: SecureSendPackage) -> None:
    ...
    if row.one_download_only:
        revoke_recipient_sessions(db, row.id, commit=False)
    db.commit()
    write_audit(
        db,
        row.sender,
        "secure_send_downloaded",
        "secure_send_package",
        str(row.id),
        client_ip(request),
        category="security",
        metadata={"download_count": row.download_count},
    )
...
def download_file(
    ...
    if not item:
        raise HTTPException(404, "This secure package could not be found.")
    metadata = next(
        (value for value in decoded_files(db, row, key) if value["row"].id == item.id),
        None,
    )
    if not metadata:
        raise HTTPException(404, "This secure package could not be found.")
    content = read_file(row, item, key)
...
def download_package(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=secure-package.zip",
            "Cache-Control": "no-store",
        },
    )
