(() => {
  ...
  async function requestError(response) {
    ...
    if (response.status === 401 || response.status === 403) return "Administrator permission is required. Refresh the page and sign in again.";
    if (response.status === 422) return "Check the contact details and try again.";
    if (response.status >= 500 && typeof payload.detail === "string" && payload.detail.startsWith("Kaya cannot securely store the Web Push private key")) return payload.detail;
    if (response.status >= 500) return `The request could not be completed. Check the application logs${requestId ? ` for request ${requestId}` : ""}.`;
    if (typeof payload.detail === "string" && payload.detail === "Invalid form token") return "Your session form token expired. Refresh the page and try again.";
  ...
  async function mutate(url, method="POST", body) {
    ...
    if (!response.ok) { const error=new Error(await requestError(response)); error.status=response.status; error.requestId=response.headers.get("X-Request-ID"); throw error; }
    if (!(response.headers.get("Content-Type")||"").toLowerCase().includes("application/json")) { const error=new Error("Kaya returned an unexpected response. Refresh the page and sign in again."); error.status=response.status; throw error; }
  ...
  const deliveryHealth=admin?.querySelector("[data-delivery-health]"),deliveryHealthStatus=deliveryHealth?.querySelector("[data-delivery-health-status]");
  deliveryHealth?.addEventListener("click",async event=>{const action=event.target.closest("[data-reconciliation-action]");if(action){const row=action.closest("[data-reconciliation-failure]"),id=row.dataset.reconciliationFailure;action.disabled=true;try{await mutate(`/api/admin/notifications/reconciliation-failures/${id}`,"PUT",{action:action.dataset.reconciliationAction});row.remove();deliveryHealthStatus.textContent="Reconciliation failure updated.";}catch(error){deliveryHealthStatus.textContent=error.message;action.disabled=false;}return;}if(event.target.closest("[data-copy-delivery-health]")){try{const health=await fetch("/api/admin/notifications/delivery-health",{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("Delivery Health could not be loaded.");return response.json();});const summary={status:health.status,workers:health.workers,pending_outbox_items:health.pending_outbox_items,stuck_outbox_items:health.stuck_outbox_items,queued_deliveries:health.queued_deliveries,stuck_delivery_items:health.stuck_delivery_items,quarantined_outbox_items:health.quarantined_outbox_items,quarantined_reconciliation_items:health.quarantined_reconciliation_items,oldest_outbox_item:health.oldest_outbox_item,oldest_queued_delivery:health.oldest_queued_delivery,last_reconciliation_result:health.last_reconciliation_result};await navigator.clipboard.writeText(JSON.stringify(summary,null,2));deliveryHealthStatus.textContent="Safe diagnostic summary copied.";}catch(error){deliveryHealthStatus.textContent=error.message||"Diagnostic summary could not be copied.";}}});
  ...
  webPush?.addEventListener("click",async event=>{const test=event.target.closest("[data-push-device-test]"),remove=event.target.closest("[data-push-device-remove]");if(!test&&!remove)return;const id=(test||remove).dataset.pushDeviceTest||(test||remove).dataset.pushDeviceRemove,result=(test||remove).closest(".mobile-pwa-device-actions")?.querySelector("[data-push-device-result]");if(remove&&!window.confirm(`Remove this ${remove.dataset.pushDevicePlatform} device for ${remove.dataset.pushDeviceUser}? Other registered devices will not be affected.`))return;(test||remove).disabled=true;try{if(test){await mutate(`/api/admin/web-push/subscriptions/${id}/test`);if(result)result.textContent="Test queued";}else{await mutate(`/api/admin/web-push/subscriptions/${id}`,"DELETE");if(result)result.textContent="Device removed";setTimeout(()=>location.reload(),300);}}catch(error){if(result)result.textContent=error.message;}finally{(test||remove).disabled=false;}});
  const showWebPushMessage=message=>{if(webPushResult)webPushResult.textContent=message;};
  const openWebPushDialog=dialog=>{if(!dialog){showWebPushMessage("Web Push setup could not be opened. Refresh the page and try again.");return false;}try{if(typeof dialog.showModal==="function")dialog.showModal();else{dialog.setAttribute("open","");dialog.classList.add("dialog-fallback-open");}return true;}catch(_error){showWebPushMessage("Web Push setup could not be opened. Refresh the page and try again.");return false;}};
  const closeWebPushDialog=dialog=>{if(!dialog)return;if(typeof dialog.close==="function")dialog.close();else{dialog.removeAttribute("open");dialog.classList.remove("dialog-fallback-open");}};
  webPush?.addEventListener("click",async event=>{const opener=event.target.closest("[data-web-push-open]");if(opener){event.preventDefault();const mode=opener.dataset.webPushOpen;if(mode==="generate"||mode==="rotate"){if(!keyForm){showWebPushMessage("Web Push setup could not be initialised. Refresh the page and try again.");return;}keyForm.reset();keyForm.dataset.mode=mode;keyForm.dataset.submitting="0";keyForm.querySelector("[data-web-push-form-status]").textContent="";keyForm.querySelector("[data-web-push-dialog-title]").textContent=mode==="rotate"?"Rotate Web Push keys":"Generate Web Push keys";keyForm.querySelector("[data-web-push-warning]").textContent=mode==="rotate"?"Rotating Web Push keys invalidates existing browser subscriptions. Every affected user may need to enable Push again on each device.":"Kaya will generate a public and private P-256 VAPID key pair. The private key will be encrypted using this installation's ENCRYPTION_KEY.";keyForm.querySelector("[data-web-push-typed-confirm]").hidden=mode!=="rotate";keyForm.querySelector("[data-web-push-submit]").textContent=mode==="rotate"?"Rotate keys and revoke subscriptions":"Generate and enable Web Push";if(openWebPushDialog(keyDialog))keyForm.contact_email.focus();}else openWebPushDialog(webPush.querySelector(`[data-web-push-dialog="${mode}"]`));return;}if(event.target.closest("[data-web-push-close]")){closeWebPushDialog(event.target.closest("dialog"));return;}const action=event.target.closest("[data-web-push-action]");if(action){const name=action.dataset.webPushAction;action.disabled=true;try{await mutate(`/api/admin/web-push/${name}`,"POST",{confirmation:name.toUpperCase()});location.reload();}catch(error){showWebPushMessage(error.message);action.disabled=false;}return;}const test=event.target.closest("[data-web-push-test]");if(test){test.disabled=true;showWebPushMessage("Queueing push test…");try{const data=await mutate("/api/admin/web-push/test");showWebPushMessage(`Push test queued for ${data.queued_devices} device(s).`);}catch(error){showWebPushMessage(error.message);}finally{test.disabled=false;}}});
  keyForm?.addEventListener("submit",async event=>{event.preventDefault();if(keyForm.dataset.submitting==="1")return;const mode=keyForm.dataset.mode,body={contact_email:keyForm.contact_email.value||null,contact_url:keyForm.contact_url.value||null,installation_label:keyForm.installation_label.value||null,confirmation:mode==="rotate"?keyForm.confirmation.value:"GENERATE"},submit=keyForm.querySelector("[data-web-push-submit]"),formStatus=keyForm.querySelector("[data-web-push-form-status]"),idleLabel=mode==="rotate"?"Rotate keys and revoke subscriptions":"Generate and enable Web Push";keyForm.dataset.submitting="1";submit.disabled=true;submit.textContent=mode==="rotate"?"Rotating…":"Generating…";formStatus.textContent="Secure key generation is in progress…";try{await mutate(`/api/admin/web-push/${mode}`,"POST",body);formStatus.textContent="Web Push keys were generated securely. Refreshing configuration…";window.setTimeout(()=>location.reload(),350);}catch(error){const message=!error.status?"Kaya could not reach the Web Push configuration service. Check your connection and try again.":error.status>=500&&!error.message.startsWith("Kaya cannot securely store")?`Web Push keys could not be generated. Check the application logs${error.requestId?` for request ${error.requestId}`:""}.`:error.message;formStatus.textContent=message;showWebPushMessage(message);keyForm.dataset.submitting="0";submit.disabled=false;submit.textContent=idleLabel;}});
