...
from app.services.secret_vault import (
    ...
    decrypt_file,
    decrypt_payload,
    decrypt_portable_package,
    encrypt_file,
    encrypt_payload,
    encrypt_portable_package,
    ensure_storage,
    lock_vault,
    master_key_from_application,
    master_key_from_pin,
    master_key_from_recovery,
...
ITEM_TYPES = {
    ...
    "sensitive_data": "Sensitive Data Record",
...
def oidc_vault_context(db: Session, request: Request, user: User, purpose: str) -> dict:
    ...
    return {
        ...
        "oidc_allowed": bool(
            provider and provider.is_enabled and policy in {"idp_mfa", "either"}
        ),
        "oidc_approved": approved,
        "totp_allowed": bool(
            user.totp_enabled and (not identity or policy in {"kaya_totp", "either"})
        ),
...
def safe_audit(
    ...
    **metadata,
    ...
    return write_audit(
        ...
        detail="Secret Vault security operation",
        category="security",
...
def item_payload(master_key: bytes, item: VaultItem) -> dict:
    return decrypt_payload(
        master_key, item.vault_id, f"item:{item.id}", item.encrypted_payload
    )


def collection_payload(master_key: bytes, row: VaultCollection) -> dict:
    return decrypt_payload(
        master_key, row.vault_id, f"collection:{row.id}", row.encrypted_payload
    )


def accessible_item(
    db: Session, user: User, item_id: int
) -> tuple[VaultItem, Vault, bytes, str]:
    ...
    if not permission and item.collection_id:
        member = (
            db.query(VaultCollectionMember)
            .filter_by(collection_id=item.collection_id, user_id=user.id)
            .first()
        )
            ...
            result.append(
                {
                    ...
                    "label": str(row["label"]).strip()[:120],
                    "value": str(row.get("value", ""))[:20000],
                    "sensitivity": (
                        sensitivity
                        if sensitivity in {"normal", "masked", "highly_sensitive"}
                        else "normal"
                    ),
                }
            )
...
def home(
    ...
    if not vault:
        return templates.TemplateResponse(
            request,
            "secret_vault_setup.html",
            base_context(
                request,
                user,
                error=None,
                **oidc_vault_context(db, request, user, "setup"),
            ),
        )
    if not vault.recovery_confirmed_at:
        return templates.TemplateResponse(
            request, "secret_vault_setup_incomplete.html", base_context(request, user)
        )
    session = active_vault_session(db, request, user, touch=False)
    if not session:
        return templates.TemplateResponse(
            request,
            "secret_vault_unlock.html",
            base_context(
                request,
                user,
                vault=vault,
    ...
    rows = (
        db.query(VaultItem)
        .filter(VaultItem.vault_id == vault.id, VaultItem.deleted_at.is_(None))
    ...
    for row in rows:
        ...
        if view == "favourites" and not row.is_favourite:
            continue
        if view == "expiring" and (
            not expiry or expiry > (date.today() + timedelta(days=30)).isoformat()
        ):
            continue
        decoded.append({"row": row, "payload": payload})
    collections = []
    for row in (
        db.query(VaultCollection)
        .filter_by(vault_id=vault.id)
        .order_by(VaultCollection.updated_at.desc())
        .all()
    ):
        try:
                ...
                shared.append(
                    {
                        ...
                        "payload": collection_payload(
                            master_key_from_application(owner_vault), row
                        ),
    ...
    activity = (
        db.query(AuditLog)
        .filter(AuditLog.user_id == user.id, AuditLog.entity.like("vault%"))
        .order_by(AuditLog.created_at.desc())
        .limit(100)
        .all()
    )
    backups = (
        db.query(VaultBackupRecord)
        .filter_by(vault_id=vault.id)
        .order_by(VaultBackupRecord.created_at.desc())
        .limit(20)
        .all()
    )
    ...
    vault_health = {
        ...
        "mfa": sensitive_auth["totp_allowed"] or sensitive_auth["oidc_allowed"],
        "portable_backup_verified": any(row.status == "verified" for row in backups),
    }
    return templates.TemplateResponse(
        request,
        "secret_vault.html",
...
def setup(
    ...
    error = validate_pin(
        pin, int(get_site_setting(db, "secret_vault_min_pin_length") or 8)
    )
    oidc_approved = consume_oidc_approval(request, user, "setup")
    auth_context = oidc_vault_context(db, request, user, "setup")
    local_approved = bool(
        user.password_hash
        and auth_context["totp_allowed"]
        and verify_password(password, user.password_hash)
        and verify_fresh_totp(db, user, totp_code)
    )
    if pin != pin_confirm:
    ...
    safe_audit(
        ...
        "vault",
        vault.id,
        authentication_method="oidc_mfa" if oidc_approved else "password_totp",
    )
    return templates.TemplateResponse(
        request,
        "secret_vault_recovery_kit.html",
        base_context(request, user, vault=vault, recovery_key=recovery_key),
    )
...
def confirm_setup(
    ...
    try:
        ...
        return templates.TemplateResponse(
            request,
            "secret_vault_setup_incomplete.html",
            base_context(
                request, user, error="Recovery key confirmation did not match."
            ),
            status_code=400,
        )
    vault.recovery_confirmed_at = datetime.utcnow()
    db.commit()
...
def unlock(
    ...
    try:
        if valid:
            ...
            valid = oidc_approved or (
                oidc_vault_context(db, request, user, "unlock")["totp_allowed"]
                and verify_fresh_totp(db, user, totp_code)
            )
    except VaultCryptoError:
        valid = False
    if not valid:
        ...
        safe_audit(
            ...
            "vault",
            vault.id,
            failure_count=vault.failed_attempts,
        )
        return templates.TemplateResponse(
            request,
            "secret_vault_unlock.html",
            base_context(
                ...
                vault=vault,
                locked_until=vault.locked_until,
                error="The vault could not be unlocked. Check your details and try again.",
                **oidc_vault_context(db, request, user, "unlock"),
            ),
            status_code=400,
        )
    method = "pin_oidc_mfa" if oidc_approved else "pin_totp"
    clear_failed_unlock(db, vault)
    start_vault_session(db, request, vault, user, method=method)
    safe_audit(
        db, user, "vault_unlocked", "vault", vault.id, authentication_method=method
    )
    return RedirectResponse("/security/secret-vault", status_code=303)


@router.post("/oidc/verify")
async def begin_oidc_verification(
    request: Request,
    purpose: str = Form(...),
    return_to: str = Form(""),
    csrf_token: str = Form(...),
    ...
    if not context["oidc_allowed"] or not provider:
        raise HTTPException(
            403, "Identity-provider MFA is not enabled for Secret Vault"
        )
    ...
    if not (
        ...
        or requested_return.startswith("/security/secret-vault/")
        or requested_return.startswith("/security/secret-vault?")
    ):
        requested_return = "/security/secret-vault"
    return await _begin(
        ...
        db,
        provider,
        flow_type=f"vault_{purpose}",
        target_user_id=user.id,
        initiated_by_user_id=user.id,
        return_path=(
            "/security/secret-vault/recover-authenticator"
            if purpose == "recovery"
            else requested_return
        ),
...
def lock(
    ...
    if vault:
        safe_audit(db, user, "vault_locked", "vault", vault.id)
    return RedirectResponse("/security/secret-vault", status_code=303)
...
def new_item(
    ...
    return templates.TemplateResponse(
        request,
        "secret_vault_item_form.html",
        base_context(
            request,
            user,
            item=None,
            payload=None,
            selected_type=item_type if item_type in ITEM_TYPES else "secure_note",
            collections=choices,
        ),
    )
...
async def create_item(
    ...
    item_type: str = Form("secure_note"),
    ...
    if item_type not in ITEM_TYPES or not title.strip():
        raise HTTPException(400, "Invalid vault item")
    collection = int(collection_id) if collection_id.isdigit() else None
    if (
        collection
        and not db.query(VaultCollection)
        .filter_by(id=collection, vault_id=vault.id)
        .first()
    ):
    ...
    payload = {
        ...
        "tags": [x.strip()[:40] for x in tags.split(",") if x.strip()][:30],
        "classification": classification[:40],
        "expiry_date": expiry_date or None,
        "review_date": review_date or None,
    }
    row.encrypted_payload = encrypt_payload(key, vault.id, f"item:{row.id}", payload)
    db.commit()
    if attachment and attachment.filename:
        await _save_attachment(db, vault, key, row, attachment)
    safe_audit(
        db, user, "vault_item_created", "vault_item", row.id, item_type=item_type
    )
    return RedirectResponse(f"/security/secret-vault/items/{row.id}", status_code=303)


async def _save_attachment(
    db: Session, vault: Vault, key: bytes, item: VaultItem, upload: UploadFile
):
    ...
    encrypted = encrypt_file(key, vault.id, storage_id, content)
    path = ensure_storage() / storage_id
    path.write_bytes(encrypted)
    metadata = {
        "name": Path(upload.filename or "attachment").name[:255],
        "content_type": (upload.content_type or "application/octet-stream")[:120],
    }
    row = VaultAttachment(
        item_id=item.id,
        storage_id=storage_id,
        encrypted_metadata=encrypt_payload(
            key, vault.id, f"attachment-meta:{storage_id}", metadata
        ),
...
def item_detail(
    ...
    for row in db.query(VaultAttachment).filter_by(item_id=item.id).all():
        attachments.append(
            {
                "row": row,
                "metadata": decrypt_payload(
                    key,
                    item.vault_id,
                    f"attachment-meta:{row.storage_id}",
                    row.encrypted_metadata,
                ),
            }
        )
    versions = (
        db.query(VaultItemVersion)
        .filter_by(item_id=item.id)
        .order_by(VaultItemVersion.version.desc())
        .all()
    )
    return templates.TemplateResponse(
        request,
        "secret_vault_item.html",
        base_context(
            request,
            user,
            item=item,
...
def edit_item(
    ...
    choices = [
        {"row": row, "payload": collection_payload(key, row)}
        for row in db.query(VaultCollection).filter_by(vault_id=owner_vault.id).all()
    ]
    return templates.TemplateResponse(
        request,
        "secret_vault_item_form.html",
...
async def update_item(
    ...
    item.encrypted_payload = encrypt_payload(key, vault.id, f"item:{item.id}", payload)
    item.updated_by_id = user.id
    item.updated_at = datetime.utcnow()
    db.commit()
    if attachment and attachment.filename:
        await _save_attachment(db, vault, key, item, attachment)
    safe_audit(
        db,
        user,
        "vault_item_updated",
...
def delete_item(
    ...
    db.commit()
    safe_audit(db, user, "vault_item_deleted", "vault_item", item.id)
    return RedirectResponse("/security/secret-vault", status_code=303)
...
def reveal_field(
    ...
    validate_csrf_token(request, csrf_token)
    require_unlocked(db, request, user)
    item, _vault, key, _permission = accessible_item(db, user, item_id)
    payload = item_payload(key, item)
    field = next(
        (x for x in payload.get("fields", []) if x.get("id") == field_id), None
    )
    if not field or field.get("sensitivity") == "normal":
        raise HTTPException(404, "Protected field not found")
    if field.get("sensitivity") == "highly_sensitive":
        own_vault = vault_for_user(db, user)
        try:
            valid = bool(
                own_vault
                and master_key_from_pin(own_vault, pin)
                and (
                    consume_oidc_approval(request, user, "sensitive")
                    or (
                        oidc_vault_context(db, request, user, "sensitive")[
                            "totp_allowed"
                        ]
        ...
        if not valid:
            ...
            return JSONResponse(
                {"error": "Fresh authentication failed"}, status_code=403
            )
    safe_audit(
        db,
        user,
        "vault_field_revealed",
        "vault_item",
        item.id,
...
def favourite(
    ...
    db.commit()
    safe_audit(db, user, "vault_favourite_changed", "vault_item", item.id)
    return RedirectResponse(f"/security/secret-vault/items/{item.id}", status_code=303)
...
def download_attachment(
    ...
    require_unlocked(db, request, user)
    attachment = db.get(VaultAttachment, attachment_id)
    if not attachment:
        raise HTTPException(404, "Attachment not found")
    item, vault, key, permission = accessible_item(db, user, attachment.item_id)
    if permission == "viewer":
        raise HTTPException(403, "Download permission required")
    path = ensure_storage() / attachment.storage_id
    encrypted = path.read_bytes()
    if not secrets.compare_digest(
        hashlib.sha256(encrypted).hexdigest(), attachment.integrity_hash
    ):
        raise HTTPException(
            409, f"Attachment integrity check failed. Reference: {safe_reference()}"
        )
    metadata = decrypt_payload(
        key,
        vault.id,
        f"attachment-meta:{attachment.storage_id}",
        attachment.encrypted_metadata,
    )
    content = decrypt_file(key, vault.id, attachment.storage_id, encrypted)
    safe_audit(
        db, user, "vault_attachment_downloaded", "vault_attachment", attachment.id
    )
...
def create_collection(
    ...
    validate_csrf_token(request, csrf_token)
    vault, key = require_unlocked(db, request, user)
    row = VaultCollection(vault_id=vault.id, encrypted_payload="pending")
    db.add(row)
    db.flush()
    row.encrypted_payload = encrypt_payload(
        key,
        vault.id,
        f"collection:{row.id}",
        {
    ...
    db.commit()
    safe_audit(db, user, "vault_collection_created", "vault_collection", row.id)
...
def share_collection(
    ...
    validate_csrf_token(request, csrf_token)
    vault, _ = require_unlocked(db, request, user)
    if get_site_setting(db, "secret_vault_sharing_enabled") != "1":
        raise HTTPException(403, "Vault sharing is disabled")
    row = (
        db.query(VaultCollection).filter_by(id=collection_id, vault_id=vault.id).first()
    )
    recipient = (
        db.query(User)
        .filter(User.email == email.strip().lower(), User.is_active)
        .first()
    )
    try:
        valid = bool(
            ...
            and permission in MEMBER_LEVELS
            and master_key_from_pin(vault, pin)
            and (
                consume_oidc_approval(request, user, "sensitive")
                or (
                    oidc_vault_context(db, request, user, "sensitive")["totp_allowed"]
                    and verify_fresh_totp(db, user, totp_code)
                )
            )
        )
    ...
    member = (
        db.query(VaultCollectionMember)
        .filter_by(collection_id=row.id, user_id=recipient.id)
        .first()
    )
    if not member:
        member = VaultCollectionMember(collection_id=row.id, user_id=recipient.id)
        db.add(member)
...
def portable_export(db: Session, vault: Vault, key: bytes) -> dict:
    ...
    for row in db.query(VaultItem).filter_by(vault_id=vault.id).all():
        ...
        for version in (
            db.query(VaultItemVersion)
            .filter_by(item_id=row.id)
            .order_by(VaultItemVersion.version.asc())
            .all()
        ):
            item["versions"].append(
                {
                    ...
                    "payload": decrypt_payload(
                        key, vault.id, f"item:{row.id}", version.encrypted_payload
                    ),
                }
            )
        ...
        items.append(item)
    collections = [
        {"id": row.id, "payload": collection_payload(key, row)}
        for row in db.query(VaultCollection).filter_by(vault_id=vault.id).all()
    ]
    return {
        "format": "kayavault",
        "version": 1,
...
def export_vault(
    ...
    validate_csrf_token(request, csrf_token)
    vault, _ = require_unlocked(db, request, user)
    if len(export_passphrase) < 12:
        raise HTTPException(
            400, "Export passphrase must contain at least 12 characters"
        )
    try:
        key = master_key_from_pin(vault, pin)
        valid = consume_oidc_approval(request, user, "sensitive") or (
            oidc_vault_context(db, request, user, "sensitive")["totp_allowed"]
            and verify_fresh_totp(db, user, totp_code)
        )
    except VaultCryptoError:
        valid = False
    if not valid:
        safe_audit(db, user, "vault_export_failed", "vault", vault.id)
        raise HTTPException(403, "Fresh authentication failed")
    package = encrypt_portable_package(
        portable_export(db, vault, key), export_passphrase
    )
    ...
    return Response(
        package,
        media_type="application/vnd.kaya.vault+json",
        headers={
            "Content-Disposition": f'attachment; filename="kaya-vault-{datetime.utcnow():%Y%m%d}.kayavault"',
            "Cache-Control": "no-store",
        },
    )
...
async def restore_vault(
    ...
    if len(raw) > 250 * 1024 * 1024:
        raise HTTPException(413, "Vault package is too large")
    ...
    try:
        ...
        for source in decoded.get("collections", []):
            ...
            if not isinstance(payload, dict):
                raise VaultCryptoError("Collection payload is invalid")
            row = VaultCollection(vault_id=vault.id, encrypted_payload="pending")
            db.add(row)
            db.flush()
            row.encrypted_payload = encrypt_payload(
                key, vault.id, f"collection:{row.id}", payload
            )
            collection_map[source.get("id")] = row.id
        for source in decoded.get("items", []):
            payload = source.get("payload")
            item_type = source.get("type")
            if not isinstance(payload, dict) or item_type not in ITEM_TYPES:
                raise VaultCryptoError("Item payload is invalid")
            row = VaultItem(
                vault_id=vault.id,
                collection_id=collection_map.get(source.get("collection_id")),
                item_type=item_type,
                encrypted_payload="pending",
                is_favourite=bool(source.get("favourite")),
                created_by_id=user.id,
                updated_by_id=user.id,
            )
            db.add(row)
            db.flush()
            row.encrypted_payload = encrypt_payload(
                key, vault.id, f"item:{row.id}", payload
            )
            for source_version in source.get("versions", []):
                version_payload = source_version.get("payload")
                if not isinstance(version_payload, dict):
                    raise VaultCryptoError("Item version payload is invalid")
                db.add(
                    VaultItemVersion(
                        item_id=row.id,
                        version=int(source_version.get("version") or 1),
                        encrypted_payload=encrypt_payload(
                            key, vault.id, f"item:{row.id}", version_payload
                        ),
                        saved_by_id=user.id,
                    )
                )
            for source_attachment in source.get("attachments", []):
                content = base64.b64decode(
                    source_attachment.get("content", ""), validate=True
                )
                if not secrets.compare_digest(
                    hashlib.sha256(content).hexdigest(),
                    source_attachment.get("sha256", ""),
                ):
                    raise VaultCryptoError("Attachment hash validation failed")
                storage_id = uuid4().hex
                encrypted = encrypt_file(key, vault.id, storage_id, content)
                attachment_path = ensure_storage() / storage_id
                attachment_path.write_bytes(encrypted)
        ...
        raise HTTPException(
            400,
            f"Restore stopped because package authentication or integrity validation failed. Reference: {safe_reference()}",
        )
...
def recover_authenticator_page(
    ...
    return templates.TemplateResponse(
        request,
        "secret_vault_authenticator_recovery.html",
        base_context(
            request,
            user,
            vault=vault,
...
def recover(
    ...
    vault = vault_for_user(db, user)
    error = validate_pin(
        new_pin, int(get_site_setting(db, "secret_vault_min_pin_length") or 8)
    )
    try:
        key = master_key_from_recovery(vault, recovery_key) if vault else None
    except VaultCryptoError:
        key = None
    ...
    if (
        error
        or new_pin != new_pin_confirm
        or not key
        or not (oidc_approved or local_approved)
    ):
        safe_audit(
            db,
            user,
            "vault_authenticator_recovery_failed",
            "vault",
            vault.id if vault else None,
        )
        return templates.TemplateResponse(
            request,
            "secret_vault_authenticator_recovery.html",
            base_context(
                ...
                user,
                vault=vault,
                error="Recovery could not be completed. Check the recovery key and identity verification.",
                **oidc_vault_context(db, request, user, "recovery"),
            ),
            status_code=400,
        )
    ...
    return templates.TemplateResponse(
        ...
        base_context(
            ...
            user,
            vault=vault,
            recovery_key=replacement_recovery_key,
...
def update_settings(
    ...
    validate_csrf_token(request, csrf_token)
    vault, _ = require_unlocked(db, request, user)
    if auto_lock_minutes not in {5, 10, 15, 30, 60}:
        raise HTTPException(400, "Invalid auto-lock interval")
    maximum = int(get_site_setting(db, "secret_vault_max_auto_lock_minutes") or 60)
    vault.auto_lock_minutes = min(auto_lock_minutes, maximum)
    db.commit()
    safe_audit(db, user, "vault_settings_changed", "vault", vault.id)
