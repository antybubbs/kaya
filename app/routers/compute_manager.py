...
router = APIRouter(
    ...
    dependencies=[Depends(require_module_access("compute_manager"))],
)
...
def workload_addresses(row):
    ...
    def add(value, label=None):
        ...
        if not address or address in seen:
            return
        seen.add(address)
        found.append({"address": address, "label": label})
    ...
    for item in raw:
        if isinstance(item, dict):
            add(
                item.get("address") or item.get("ip_address") or item.get("ip-address"),
                item.get("network") or item.get("interface") or item.get("name"),
            )
        else:
            add(item)
    ...
    if isinstance(networks, dict):
        for name, network in networks.items():
            network = network or {}
            values = network.get("addresses") or [
                network.get("IPAddress"),
                network.get("GlobalIPv6Address"),
...
def workload_network_context(db, rows):
    result = {row.id: workload_addresses(row) for row in rows}
    addresses = {item["address"] for items in result.values() for item in items}
    records = {}
    if addresses:
    ...
    for items in result.values():
        for item in items:
            item["record"] = records.get(item["address"])
    return result
...
def overview(
    ...
    workloads = (
        query.order_by(ComputeWorkload.status.asc(), ComputeWorkload.name.asc())
        .limit(500)
        .all()
    )
    hosts = db.query(ComputeHost).order_by(ComputeHost.name).all()
    items = (
        db.query(ComputeInventoryItem)
        .order_by(ComputeInventoryItem.kind, ComputeInventoryItem.name)
        .all()
    )
    events = (
        db.query(ComputeEvent).order_by(ComputeEvent.created_at.desc()).limit(12).all()
    )
    return templates.TemplateResponse(
        request,
        "compute_manager.html",
        context(
            user=user,
            hosts=hosts,
            workloads=workloads,
    ...
    summary["hosts"] = [
        {
            ...
            "status": h.status,
            "cpu": h.cpu_percent,
            "memory": pct(h.memory_used, h.memory_total),
            "storage": pct(h.storage_used, h.storage_total),
            "updated_at": h.last_synced_at.isoformat() if h.last_synced_at else None,
            "error": h.last_error,
        }
        for h in db.query(ComputeHost).order_by(ComputeHost.name)
    ]
    summary["workloads"] = [
        {
            "id": w.id,
            ...
            "status": w.status,
            "cpu": w.cpu_percent,
            "memory": pct(w.memory_used, w.memory_total),
            "storage": pct(w.storage_used, w.storage_total),
            "owner": w.owner,
        }
        for w in db.query(ComputeWorkload)
        .filter(ComputeWorkload.status != "missing")
        .order_by(ComputeWorkload.name)
    ]
...
def new_host(request: Request, user=Depends(require_editor)):
    return templates.TemplateResponse(
        request,
        "compute_host_form.html",
        context(user=user, host=None, error=None, **csrf_context(request)),
    )
...
def create_host(
    ...
    platform: str = Form(...),
    base_url: str = Form("", max_length=500),
    token_id: str = Form("", max_length=255),
    token_secret: str = Form("", max_length=2000),
    verify_tls: str = Form(""),
    is_enabled: str = Form(""),
    ...
    elif platform == "proxmox" and (not token_id.strip() or not token_secret.strip()):
        error = "Proxmox requires an API token ID and secret."
    elif db.query(ComputeHost).filter(ComputeHost.name == clean_name).first():
...
def render_host_detail(
    ...
    workloads = (
        ...
        .order_by(ComputeWorkload.kind, ComputeWorkload.name)
        .all()
    )
    items = (
        db.query(ComputeInventoryItem)
    ...
    metrics = (
        ...
        .filter(ComputeMetric.host_id == host.id, ComputeMetric.workload_id.is_(None))
...
def edit_host(
    ...
    return templates.TemplateResponse(
        request,
        "compute_host_form.html",
        context(user=user, host=host, error=None, **csrf_context(request)),
    )
...
def update_host(
    ...
    host.base_url = (
        f"agent://{host.name}" if platform == "docker_agent" else base_url.strip()
    )
    ...
    host.owner = owner.strip() or None
    ...
    return RedirectResponse(
        f"/infrastructure/vm-docker-manager/hosts/{host.id}", status_code=303
    )
...
def delete_host(
    ...
    name = host.name
    workload_ids = [
        x.id for x in db.query(ComputeWorkload.id).filter_by(host_id=host.id)
    ]
    db.query(BackupJob).filter(BackupJob.host_id == host.id).delete(
        synchronize_session=False
    )
    if workload_ids:
        db.query(ComputeMetric).filter(
            ComputeMetric.workload_id.in_(workload_ids)
        ).delete(synchronize_session=False)
    for model in (ComputeMetric, ComputeEvent, ComputeInventoryItem, ComputeWorkload):
        db.query(model).filter(model.host_id == host.id).delete(
            synchronize_session=False
        )
    db.delete(host)
    db.commit()
...
def workload_detail(
    ...
    metrics = (
        db.query(ComputeMetric)
        .filter_by(workload_id=row.id)
        .order_by(ComputeMetric.recorded_at.desc())
        .limit(120)
        .all()[::-1]
    )
    events = (
        db.query(ComputeEvent)
        .filter_by(workload_id=row.id)
        .order_by(ComputeEvent.created_at.desc())
        .limit(50)
        .all()
    )
...
def update_workload(
    ...
    return RedirectResponse(
        f"/infrastructure/vm-docker-manager/workloads/{row.id}", status_code=303
    )
