...
def dashboard(
    ...
    return templates.TemplateResponse(
        request,
        "dashboard.html",
        {"user": user, "dashboard_config": dashboard_config, **csrf_context(request)},
    )
...
def dashboard_snapshot(
    response: Response, db: Session = Depends(get_db), user=Depends(require_user)
):
...
def update_dashboard_preferences(
    ...
    try:
        return {"layout": save_preferences(db, user, payload)}
