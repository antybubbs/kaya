...
STORAGE = Path(
    os.getenv("KAYA_SECURE_SEND_STORAGE_DIR", str(DEFAULT_DATA_DIR / "secure-send"))
)
if not DEFAULT_DATA_DIR.exists():
    STORAGE = PROJECT_ROOT / "data" / "secure-send"
...
def gateway_health(*, force: bool = False) -> dict[str, str]:
    ...
    with _GATEWAY_HEALTH_LOCK:
        ...
        if (
            not force
            and cached
            and float(_GATEWAY_HEALTH_CACHE.get("expires") or 0) > now
        ):
            return dict(cached)
        result = {
            "state": "unavailable",
...
def decrypt_value(key: bytes, package_id: int, purpose: str, value: str) -> Any:
    try:
        packed = _unb64(value)
        return json.loads(
            AESGCM(key).decrypt(packed[:12], packed[12:], _aad(package_id, purpose))
        )
    except (
        InvalidTag,
        ValueError,
...
def encrypt_file(key: bytes, package_id: int, storage_id: str, content: bytes) -> bytes:
    ...
    return nonce + AESGCM(key).encrypt(
        nonce, content, _aad(package_id, f"file:{storage_id}")
    )
...
def decrypt_file(key: bytes, package_id: int, storage_id: str, content: bytes) -> bytes:
    try:
        return AESGCM(key).decrypt(
            content[:12], content[12:], _aad(package_id, f"file:{storage_id}")
        )
...
def validate_pin(pin: str) -> str | None:
    ...
    if clean.isdigit() and (
        len(set(clean)) == 1
        or clean in "01234567890123456789"
        or clean in "98765432109876543210"
    ):
...
def create_package(
    ...
    *,
    recipient_type: str,
    internal_recipient_id: int | None,
    summary: dict,
    note: str,
    pin: str,
    expires_at: datetime,
    one_download_only: bool,
    allow_vault_save: bool,
    notify_when_opened: bool,
    ...
    row = SecureSendPackage(
        sender_id=sender.id,
        internal_recipient_id=internal_recipient_id,
        recipient_type=recipient_type,
        access_token_hash=hashlib.sha256(access_token.encode()).hexdigest(),
        encrypted_access_token=encrypt_secret(access_token),
        credential_hash=hash_password(material),
        credential_salt=_b64(salt),
        credential_wrapped_key=wrap_key(
            key, derive_wrapping_key(material, salt), "secure-send-credentials"
        ),
        app_wrapped_key=wrap_key(key, application_kek(), "secure-send-application"),
        encrypted_summary="pending",
        encrypted_note=None,
        expires_at=expires_at,
        one_download_only=one_download_only,
        allow_vault_save=allow_vault_save,
        notify_when_opened=notify_when_opened,
    )
    db.add(row)
    db.flush()
        ...
        for name, content_type, content in files:
            ...
            db.add(
                SecureSendFile(
                    ...
                    encrypted_metadata=encrypt_value(
                        key,
                        row.id,
                        f"file-meta:{storage_id}",
...
def package_for_token(db: Session, access_token: str) -> SecureSendPackage | None:
    ...
    return (
        ...
        .filter_by(access_token_hash=hashlib.sha256(access_token.encode()).hexdigest())
        .first()
    )
...
def package_accessible(row: SecureSendPackage, now: datetime | None = None) -> bool:
    ...
    return bool(
        row.status not in {"expired", "revoked", "deleted"}
        and not row.revoked_at
        and not row.deleted_at
        and row.expires_at > now
        and not row.cleaned_at
    )


def authenticate_package(
    row: SecureSendPackage, access_token: str, pin: str, passphrase: str
) -> bytes:
    material = credential_material(access_token, pin, passphrase)
    ...
    return unwrap_key(
        ...
        derive_wrapping_key(material, _unb64(row.credential_salt)),
        "secure-send-credentials",
    )


def decode_summary(row: SecureSendPackage, key: bytes | None = None) -> dict:
    value = decrypt_value(
        key or package_key_from_application(row),
        row.id,
        "summary",
...
def decode_note(row: SecureSendPackage, key: bytes) -> str:
    return (
        str(decrypt_value(key, row.id, "note", row.encrypted_note))
...
def decoded_files(db: Session, row: SecureSendPackage, key: bytes) -> list[dict]:
    result = []
    for item in (
        db.query(SecureSendFile)
        .filter_by(package_id=row.id)
        .order_by(SecureSendFile.id)
        .all()
    ):
        metadata = decrypt_value(
            key, row.id, f"file-meta:{item.storage_id}", item.encrypted_metadata
        )
        result.append(
            {
                "row": item,
                "name": str(metadata.get("name") or "secure-file"),
                "content_type": str(
...
def read_file(row: SecureSendPackage, item: SecureSendFile, key: bytes) -> bytes:
    ...
    if not secrets.compare_digest(
        hashlib.sha256(encrypted).hexdigest(), item.integrity_hash
    ):
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        for index, item in enumerate(decoded_files(db, row, key), 1):
            ...
            if name in used:
                name = f"{index}-{name}"
            used.add(name)
            archive.writestr(name, read_file(row, item["row"], key))
        note = decode_note(row, key)
        if note:
            archive.writestr("secure-note.txt", note)
...
def record_activity(
    ...
    *,
    ...
    activity = SecureSendActivity(
        package_id=row.id,
        event_type=event_type,
        actor_user_id=actor_user_id,
        encrypted_detail=None,
    )
    db.add(activity)
    db.flush()
    if detail:
        activity.encrypted_detail = encrypt_value(
            package_key_from_application(row), row.id, f"activity:{activity.id}", detail
        )
    if commit:
        db.commit()
    return activity
...
def start_recipient_session(
    db: Session, row: SecureSendPackage
) -> tuple[str, str, SecureSendRecipientSession]:
    ...
    session = SecureSendRecipientSession(
        ...
        expires_at=min(row.expires_at, now + timedelta(minutes=SESSION_MINUTES)),
    )
    db.add(session)
    db.commit()
...
def active_recipient_session(
    db: Session, row: SecureSendPackage, token: str | None
) -> SecureSendRecipientSession | None:
    if not token or not package_accessible(row):
        return None
    now = datetime.utcnow()
    session = (
        db.query(SecureSendRecipientSession)
        .filter_by(
            package_id=row.id, token_hash=hashlib.sha256(token.encode()).hexdigest()
        )
        .first()
    )
    ...
    session.expires_at = min(row.expires_at, now + timedelta(minutes=SESSION_MINUTES))
    db.commit()
    return session
...
def verify_session_csrf(session: SecureSendRecipientSession, value: str) -> bool:
    return bool(
        ...
        and secrets.compare_digest(
            session.csrf_hash, hashlib.sha256(value.encode()).hexdigest()
        )
    )


def revoke_recipient_sessions(
    db: Session, package_id: int, *, commit: bool = True
) -> None:
    db.query(SecureSendRecipientSession).filter_by(
        package_id=package_id, revoked_at=None
    ).update(
        {SecureSendRecipientSession.revoked_at: datetime.utcnow()},
...
def clean_package_content(db: Session, row: SecureSendPackage, *, status: str) -> None:
    ...
    for item in db.query(SecureSendFile).filter_by(package_id=row.id).all():
        try:
            (ensure_storage() / item.storage_id).unlink(missing_ok=True)
        except OSError:
            pass
        db.delete(item)
    row.encrypted_note = None
    row.cleaned_at = now
    row.status = status
    if status == "expired":
...
def expire_and_cleanup(db: Session) -> int:
    ...
    rows = (
        db.query(SecureSendPackage)
        .filter(
            SecureSendPackage.expires_at <= now, SecureSendPackage.cleaned_at.is_(None)
        )
        .all()
    )
    for row in rows:
        record_activity(db, row, "expired", commit=False)
        clean_package_content(db, row, status="expired")
        count += 1
    return count
