...
def _checkin_workloads(db, host, key, private, workloads):
    identity = db.query(BackupAgentIdentity).filter_by(host_id=host.id).one()
    response = asyncio.run(
        checkin(
            signed_request(
                ...
                identity.id,
                key.key_id,
                body={"host": {}, "workloads": workloads, "items": []},
            ),
            db,
        )
    )
...
def test_v2_checkin_persists_workload_metrics_and_summary_values():
    with database() as db:
...
def test_v2_checkin_preserves_zero_and_missing_metrics():
    with database() as db:
        ...
        _checkin_workloads(
            ...
            private,
            [
                {"name": "zero-cpu", "cpu_percent": 0.0},
                {"name": "zero-memory", "memory_used": 0, "memory_total": 3_980_000_000},
                {"name": "unknown"},
            ],
        )
        rows = {row.name: row for row in db.query(ComputeWorkload).filter_by(host_id=host.id).all()}
        assert rows["zero-cpu"].cpu_percent == 0.0
        assert rows["zero-memory"].memory_used == 0
        assert rows["zero-memory"].memory_total == 3_980_000_000
        assert rows["unknown"].cpu_percent is None
        assert rows["unknown"].memory_used is None


def test_v2_repeated_checkin_reconciles_workload_and_updates_current_values():
    with database() as db:
        host, _, key, private = enrolled(db)
        _checkin_workloads(db, host, key, private, [{"name": "repeat", "cpu_percent": 1.0, "memory_used": 10}])
        _checkin_workloads(db, host, key, private, [{"name": "repeat", "cpu_percent": 2.0, "memory_used": 20}])
        rows = db.query(ComputeWorkload).filter_by(host_id=host.id, name="repeat").all()
        assert len(rows) == 1
