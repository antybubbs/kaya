...
def user(db, email="viewer@example.test", role="viewer"):
    item=User(email=email,password_hash="x",role=role,is_active=True,created_at=datetime.utcnow())
    db.add(item)
    db.flush()
    grant_all_registered_modules(db,item)
    db.commit()
    return item

def test_preferences_are_per_user_and_persist(db):
    first=user(db)
    second=user(db,"second@example.test")
    layout=default_layout(db,first)
    layout["widgets"][0]["width"]="medium"
...
def test_invalid_widget_key_size_and_position_are_rejected(db):
    account=user(db)
    layout=default_layout(db,account)
    bad=json.loads(json.dumps(layout))
    bad["widgets"][0]["key"]="ssl_certificates"
    with pytest.raises(ValueError,match="Unknown"):
        normalise_layout(db,account,bad)
    bad=json.loads(json.dumps(layout))
    bad["widgets"][0]["width"]="giant"
    with pytest.raises(ValueError,match="size"):
        normalise_layout(db,account,bad)
    bad=json.loads(json.dumps(layout))
    bad["widgets"][0]["position"]=-1
...
def test_reset_restores_defaults(db):
    account=user(db)
    layout=default_layout(db,account)
    layout["monitor_mode"]=True
    save_preferences(db,account,layout)
    result=reset_preferences(db,account)
...
def test_restricted_widgets_are_not_returned(db):
    viewer=user(db)
    keys={item["key"] for item in config(db,viewer)["widgets"]}
...
def test_polling_interval_is_one_of_the_supported_choices(db, stored, expected):
    account=user(db)
    db.add(RemoteManagerSetting(key="dashboard_poll_interval_seconds",value=stored))
    db.commit()
    assert config(db,account)["poll_interval_seconds"] == expected

def test_enabled_dashboard_editing_controls_remain_available(db):
    account=user(db)
    db.add_all([
    ...
    assert result["customisation_enabled"] is True
    assert result["monitor_mode_enabled"] is True

def test_malformed_preferences_fall_back_safely(db):
    account=user(db)
    db.add(DashboardPreference(user_id=account.id,preference_version=99,layout_json="not json"))
    db.commit()
    assert preferences(db,account) == default_layout(db,account)

def test_widget_failure_does_not_fail_snapshot(db, monkeypatch):
    account=user(db)
    original=dashboard_service._build
